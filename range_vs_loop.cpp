...
int main( int, char* [] )
{
  ...
  auto resultLoop =  get_max_archor_level_for_loop( warriors ) ;
  auto resultRange = get_max_archor_level_range_v3( warriors ) ;

  std::cout << "resultLoop = " << resultLoop << std::endl;
  std::cout << "resultRange = " << resultRange << std::endl;
  return 0;
}
